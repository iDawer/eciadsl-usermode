...
*********************************************************************
 File		: 	$RCSfile: eciadsl-pppoeci.c,v $
 Version	:	$Revision: 1.36 $
 Modified by	:	$Author: hetfield666 $ ($Date: 2005/10/22 13:05:47 $)
 Licence	:	GPL
*********************************************************************
...
/* for ident(1) command */
static const char id[] = "@(#) $Id: eciadsl-pppoeci.c,v 1.36 2005/10/22 13:05:47 hetfield666 Exp $";
...
//		usleep(sleep1);
	...
	if (iEciPPPStatus==ERR_EOC_PROBLEM) {
		...
		strcat(reconnect_script,BINDIR);
		strcat(reconnect_script,"/eciadsl-restart");
...
void sigtimeout(){
	...
	if(rcvEciMsg(&ecimsg,0,0)>0){
		switch(ecimsg.ecicmd){
			...
			case ECI_MC_NO_ACTION:
			case ECI_MC_DO_FIRM:
			case ECI_MC_FIRMED:
			case ECI_MC_DO_SYNCH:
			case ECI_MC_SYNCHED:
			case ECI_MC_DO_CONNECT:
			case ECI_MC_CONNECTED:
			case ECI_MC_BANDWIDTH:
			case ECI_MC_DISCONNECTED:
			case ECI_MC_CMD_FAILED:
			case ECI_MC_CONNECTION_FAILED:
